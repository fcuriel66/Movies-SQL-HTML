...
# Create the engine
engine = create_engine(DB_URL, echo=False)  #echo=True used to debug only
...
def list_movies_html():
    """Retrieves all movies from database including poster"""
    with engine.connect() as connection:
...
def delete_movie(title):
    ...
    with engine.connect() as connection:
        try:
            connection.execute(text("DELETE FROM movies WHERE movies.title = :title"),
                               {"title": title})
            connection.commit()
...
#print(list_movies())
#add_movie("Gattaca",2000, 8)
#delete_movie("Gattaca")
#update_movie("Gattaca",15)